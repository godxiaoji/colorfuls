(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('big.js/big.mjs')) :
	typeof define === 'function' && define.amd ? define(['big.js/big.mjs'], factory) :
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.Colorfuls = factory(global.Big));
	...
	function bigNumberRange(value, min = 0, max = 1) {
	  if (value.gt(max)) {
	    return new Big__default['default'](max)
	  } else if (value.lt(min)) {
	    return new Big__default['default'](min)
	  }
	...
	function _hsl2rgb(h, s, l) {
	  ...
	  if (s.eq(0)) {
	    ...
	    h = new Big__default['default'](h).div(360);
	...
	function parseAlpha(value) {
	  ...
	  return new Big__default['default'](opacity)
	}

	function channelLength(value) {
	  return bigNumberRange(new Big__default['default'](percentage2Length(value)))
	}
	...
	function channelDown(channel, ratio) {
	  ...
	  return bigNumberRange(
	    channel.times(new Big__default['default'](1).minus(percentage2Length(ratio)))
	  )
	}
	...
	function channelUp(channel, ratio) {
	  ...
	  return bigNumberRange(
	    channel.times(new Big__default['default'](1).plus(percentage2Length(ratio)))
	  )
	}

	function isBig(object) {
	  return object instanceof Big__default['default']
	}
	...
	function hue2rgb(p1, p2, hue) {
	  ...
	  if (hue.times(3).lt(2))
	    return p1.plus(p2.minus(p1).times(new Big__default['default'](2 / 3).minus(hue).times(6)))
	  return p1
	}
	...
	function mix(color1, color2, weight) {
	  ...
	  const p = new Big__default['default'](
	  ...
	  const w2 = new Big__default['default'](1).minus(w1);
	  ...
	  return rgba2RGBA({
	    ...
	    a: c1._a.times(p).plus(c2._a.times(new Big__default['default'](1).minus(p)))
	  })
	}

	function color2Array(color, gamma) {
	  return color.toArray().map((channel, k) => {
	    return (k === 3 ? new Big__default['default'](channel) : new Big__default['default'](channel).div(255)).pow(gamma)
	  })
	}
	...
	function getStepColor(p, colors, gamma) {
	  ...
	  for (let i = 0; i < 4; i++) {
	    const channel = start[i]
	      .times(new Big__default['default'](1).minus(p))
	      .plus(end[i].times(p))
	      .pow(1 / gamma);
	...
	function parseColors(args) {
	  ...
	  for (let i = 0, len = args.length; i < len; i++) {
	    ...
	    if (isArray(args[i])) {
	      ...
	      item.percentage = new Big__default['default'](
	    ...
	    if (i === 0) {
	      item.percentage = new Big__default['default'](0);
	    } else if (i === len - 1) {
	      item.percentage = new Big__default['default'](1);
	    }
	...
	class Gradient {
	  ...
	  steps(length) {
	    if (isNumber(length) && length >= 2) {
	      ...
	      for (let i = 0; i < length; i++) {
	        output.push(
	          getStepColor(new Big__default['default'](i).div(length - 1), this.colors, this.gamma)
	        );
	      }
	  ...
	  step(value) {
	    ...
	    return getStepColor(new Big__default['default'](p), this.colors, this.gamma)
	  }
	}
